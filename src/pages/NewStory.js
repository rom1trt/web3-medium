...
import "./NewStory.css"
import {
  useMoralisFile, // Upload NFT metadata on IPFS
  useMoralis, // Getting the account
  useWeb3ExecuteFunction, // Execute functions on our smart contract
} from "react-moralis";
...
const NewStory = () => {
  ...
  const mint = async (account, uri) => {
    ...
    let options ={
      contractAddress: `${MediumAddress.address}`,
  ...
  const uploadFile = async (event) => {
    ...
    // Moralis function to save on IPFS
    try {
  ...
  const uploadNftMetada = async (url) => {
    const metadataNft = {
      image:
        "https://ipfs.io/ipfs/QmQA9U1xjCKAxfENc8NwGSvmcRkX5onceibQv7f15gb8fS?filename=decryptoLogo.png",
      description: title,
      externalUrl: url,
    };
